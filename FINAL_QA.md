...
- Legacy `assets/robot-portrait.svg` has been removed after confirming it was not referenced by the active source.
...
- Anchor scrolling now accounts for the sticky header.
- Keyboard focus states remain visible.
- Reduced-motion handling now bypasses reveal animation entirely and reveal observers stop observing an element after it becomes visible.
...
3. `assets/robot-portrait.svg` has been removed.
...
9. README and photo guide now describe the current photo implementation.
...
## Recommended final manual checks after deployment
- Desktop: 1440px and 1280px widths.
...
Live browser/mobile visual validation: **PENDING** because an interactive rendered browser is not available through the current GitHub connector.
